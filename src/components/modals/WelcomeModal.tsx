        ...
        switch (activeTab) {
            ...
            case 'feedback':
                return (
                    ...
                    >
                        <CTASection
                            badge={{ text: "Help us improve" }}
                            title="We Want Your Feedback"
                            description="This massive update was built entirely to supercharge your learning. If you spot a glitch, have a suggestion, or simply want to let us know what you think, please don't hesitate to share!"
                            action={{
    ...
    return createPortal(
        ...
        >
            <motion.div
                ...
                onClick={onClose}
                className={`absolute inset-0 bg-slate-900/60 sm:bg-slate-900/40 backdrop-blur-sm sm:backdrop-blur-md ${isOpen ? "pointer-events-auto" : "pointer-events-none"}`}
                style={{ willChange: 'opacity' }}
            />

            <div className={`pointer-events-none absolute inset-0 flex justify-center items-center p-4 sm:p-6`}>
                <AnimatePresence initial={false}>
                    {isOpen ? (
                        <motion.div
                            ...
                            className="pointer-events-auto relative w-full max-w-4xl lg:max-w-6xl bg-white dark:bg-slate-900 rounded-3xl shadow-2xl overflow-hidden border border-slate-200/50 dark:border-slate-700/50 flex flex-col max-h-[95vh] will-change-transform"
                        >
                            <motion.div layout="position" style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0, width: '100%' }}>
                                ...
                                <div className="p-4 sm:p-8 flex flex-col pt-10 sm:pt-10 flex-1 min-h-0 overflow-y-auto hide-scrollbar">
                                    {/* Dock Tabs */}
                                    <div className="flex items-center justify-center gap-2 sm:gap-4 mb-8 scale-95 sm:scale-100 flex-shrink-0">
