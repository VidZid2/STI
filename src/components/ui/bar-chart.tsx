...
export interface BarXAxisProps {
  ...
  maxLabels?: number;
  tickFormat?: (label: string) => React.ReactNode;
}
      ...
      {labelsToShow.map((item) => {
        ...
        return (
          ...
          >
            ...
            >
              {tickFormat ? tickFormat(item.label) : item.label}
            </motion.span>
          </div>
        );
      })}
...
function BarChartInner({
  ...
  useEffect(() => {
    // The total animation time = last bar's stagger delay + animation duration.
    // autoStagger per bar = min(0.06, 0.8 / data.length) seconds
    // last bar delay = (data.length - 1) * autoStagger seconds
    const autoStagger = Math.min(0.06, 0.8 / Math.max(1, data.length));
    const lastBarDelay = (data.length - 1) * autoStagger * 1000; // convert to ms
    const totalTime = lastBarDelay + animationDuration;
