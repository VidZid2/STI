...
const InteractiveMenu: React.FC<InteractiveMenuProps> = ({ items, accentColor }) => {
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      setActiveIndex((prev) => (prev + 1) % finalItems.length);
  ...
  useEffect(() => {
    ...
    // Need a tiny delay for CSS grid transition to compute correct scrollWidth
  ...
  return (
    <nav
      className="menu shadow-xl dark:shadow-none"
      role="navigation"
      style={navStyle}
    >
