  ...
  const modalContent = (
    ...
    >
      <motion.div
        ...
        onClick={onClose}
        className={cn(
          "absolute inset-0 bg-background/5 [backdrop-filter:blur(14px)_saturate(140%)] [-webkit-backdrop-filter:blur(14px)_saturate(140%)] bg-black/40",
          open ? "pointer-events-auto" : "pointer-events-none",
        )}
      />
        ...
        <AnimatePresence initial={false}>
          {open ? (
            <motion.div
              ...
              className={cn(
                "pointer-events-auto relative w-full overflow-hidden rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xl will-change-transform",
                className,
              )}
            >
              <motion.div layout="position" className="p-0">
