                        ...
                        <div className='flex items-center gap-0.5 sm:gap-2 lg:gap-3' ref={menuRef}>
                            {ITEMS.map((item) => (
                                <motion.button
                                    ...
                                    transition={{ type: 'spring', stiffness: 400, damping: 15 }}
                                    className={cn(
                                        'relative shrink-0 select-none appearance-none items-center justify-center transition-all duration-300 focus-visible:ring-2 shadow-sm border w-[30px] h-[30px] sm:w-10 sm:h-10 lg:w-12 lg:h-12 rounded-[8px] sm:rounded-[12px] lg:rounded-[14px]',
                                        (item.id === 3 || item.id === 4) ? 'hidden sm:flex' : 'flex',
                                        active === item.id
