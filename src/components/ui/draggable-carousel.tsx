  ...
  return (
    ...
    >
      <div className={cn('group/hover relative', className)}>
        <div className='overflow-x-clip overflow-y-visible'>{children}</div>
      </div>
    </CarouselProvider>
  );
}
...
function CarouselContent({
  ...
  const onDragEnd = (e: any, info: any) => {
    ...
    // Detect either a long drag or a fast swipe
    if ((offset <= -20 || velocity <= -500) && index < itemsLength - 1) {
      setIndex(index + 1);
    } else if ((offset >= 20 || velocity >= 500) && index > 0) {
      setIndex(index - 1);
    }
  };
