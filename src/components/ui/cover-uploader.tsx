...
export function CoverUploader({
	...
	React.useEffect(() => {
		if (open) {
			document.body.setAttribute('data-cover-uploader-open', 'true');
		} else {
			document.body.removeAttribute('data-cover-uploader-open');
		}
		return () => {
			document.body.removeAttribute('data-cover-uploader-open');
		};
	...
	return (
		<>
			...
			<MorphingModal
				viewId={open ? "cover-uploader" : null}
				onClose={() => {
					...
					<div className="flex flex-row items-start gap-4 text-left sm:text-left bg-transparent border-b-0 p-6 pb-4 relative z-10">
						...
						<div className="flex-1 min-w-0 pt-0.5">
							...
							<p className="text-[12.5px] font-medium leading-[1.4] text-zinc-500 dark:text-zinc-400">
								Choose and customize your profile cover
							</p>
						</div>
					</div>
