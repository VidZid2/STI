...
function getOptionLabel(group: PromptSettingGroup, value: string) {
  return group.options.find((option) => option.value === value)?.label ?? group.options[0]?.label ?? value;
}

function getSelectedOption(group: PromptSettingGroup, value: string) {
  return group.options.find((option) => option.value === value) ?? group.options[0];
}

const dropdownPanelClassName =
  "fixed z-[100000] w-[min(14.5rem,calc(100vw-1.5rem))] overflow-y-auto overscroll-contain rounded-xl border border-border/60 bg-white dark:bg-zinc-900 py-1 text-sm shadow-[0_10px_28px_-20px_rgba(0,0,0,0.14)] outline-none [-webkit-overflow-scrolling:touch]";
...
function DropdownMenuDivider() {
  return <div className="mx-2 my-0.5 h-px shrink-0 bg-gray-200 dark:bg-zinc-800" />;
}
  ...
  return (
    <>
      <button
        ...
        className="flex shrink-0 items-center gap-1.5 rounded-full py-1 text-sm transition-colors hover:text-foreground"
        onClick={toggleOpen}
      ...
      >
        ...
        {groups.map((group, index) => {
          ...
          return (
            <span
              className={`whitespace-nowrap font-medium ${index === 0 ? "text-foreground" : "text-muted-foreground"}`}
              key={group.id}
            >
              {label}
            </span>
      ...
      <AnimatePresence>
        {open ? (
          <motion.div
            animate={{ opacity: 1, scale: 1, y: 0 }}
            aria-label={ariaLabel}
            className={`absolute z-[100000] w-max min-w-52 max-w-[calc(100vw-2rem)] rounded-2xl border border-border bg-white dark:bg-zinc-900 p-1 shadow-[0_10px_28px_-18px_rgba(0,0,0,0.25)] ${align === "end" ? "right-0" : "left-0"} ${upward ? `bottom-full mb-2 ${align === "end" ? "origin-bottom-right" : "origin-bottom-left"}` : `top-full mt-2 ${align === "end" ? "origin-top-right" : "origin-top-left"}`}`}
...
function PlusMenu({
  ...
  const renderItem = (item: AIInputMenuItem, inSubmenu: boolean) => {
    if (item.type === "separator") {
      return <div className="mx-1 my-0.5 h-px bg-gray-200 dark:bg-zinc-800" key={item.value} />;
    }
      ...
      <AnimatePresence>
        {open ? (
          <motion.div
            animate={{ opacity: 1, scale: 1, y: 0 }}
            aria-label="More options"
            className={`absolute left-0 z-[100000] w-max min-w-64 max-w-[calc(100vw-2rem)] overflow-hidden rounded-2xl border border-border bg-white dark:bg-zinc-900 p-1 shadow-[0_10px_28px_-18px_rgba(0,0,0,0.25)] ${upward ? "bottom-full mb-2 origin-bottom-left" : "top-full mt-2 origin-top-left"}`}
            exit={{ opacity: 0, scale: 0.97, y: upward ? 4 : -4 }}
            initial={{ opacity: 0, scale: 0.96, y: upward ? 6 : -6 }}
            onMouseLeave={() => setHoveredValue(null)}
            role="menu"
            ...
            <AnimatePresence custom={slideDir} initial={false} mode="wait">
              {activeParent ? (
                ...
                >
                  ...
                  <div className="mx-1 my-0.5 h-px bg-gray-200 dark:bg-zinc-800" />
...
export function AIInput({
  ...
  return (
    <div className={`w-full ${className ?? ""}`}>
      ...
      <div className="@container relative rounded-[28px] border border-border bg-white dark:bg-zinc-900 shadow-[0_10px_26px_-22px_rgba(0,0,0,0.14)]">
        <AnimatePresence>
          {waveRun > 0 ? (
            <SendWave
        ...
        <InputPrimitive
          aria-label="Message"
          className="hide-scrollbar block max-h-[132px] w-full resize-none break-words break-all overflow-y-auto overflow-x-hidden bg-transparent px-4 pt-4 pb-1 text-base text-foreground leading-6 outline-none placeholder:text-muted-foreground sm:px-5 sm:text-sm"
          onValueChange={(next) => {
            setValue(next);
            syncHeight();
          }}
