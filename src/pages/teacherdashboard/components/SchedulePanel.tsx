...
export const SchedulePanel: React.FC<SchedulePanelProps> = ({ isLoadingSchedule, todaysSchedule }) => {
    return (
        <motion.div
            ...
            className="schedule-panel bg-surface/60 dark:bg-slate-900/60 backdrop-blur-md rounded-[20px] p-6 border border-black/5 dark:border-white/10"
        >
                ...
                {isLoadingSchedule ? (
                    ...
                    [...Array(3)].map((_, i) => (
                        <div key={i} className="flex items-center gap-4 p-4 rounded-xl">
                            <div className="w-9 h-9 rounded-lg bg-black/5 dark:bg-white/5 animate-pulse" />
                            <div className="flex-1">
                                <div className="w-[60%] h-3.5 bg-black/5 dark:bg-white/5 rounded mb-1.5 animate-pulse" />
                                <div className="w-[40%] h-3 bg-black/5 dark:bg-white/5 rounded animate-pulse" />
                            </div>
                            <div className="w-20 h-3.5 bg-black/5 dark:bg-white/5 rounded animate-pulse" />
                        </div>
                    ))
                        ...
                        return (
                            <motion.div
                                ...
                                transition={isOngoing ? {
                                    ...
                                    default: { duration: 0.15, ease: 'easeOut', delay: 0.15 + index * 0.05 }
                                } : { duration: 0.15, ease: 'easeOut', delay: 0.15 + index * 0.05 }}
                                whileHover={{
                                    scale: 1.01,
                                    background: isOngoing ? 'rgba(59, 130, 246, 0.08)' : 'var(--bg-surface-alt, rgba(255,255,255,0.02))'
                                }}
                                className={`flex items-center gap-4 p-4 rounded-xl cursor-pointer border ${
                                    isOngoing 
                                        ? 'bg-blue-500/[0.06] border-blue-500/20' 
                                        : 'bg-transparent border-transparent dark:border-white/5'
                                }`}
                            >
                                {/* Status Indicator */}
                                ...
                                <div className="text-right shrink-0">
                                    <div className={`text-xs font-medium ${isOngoing ? 'text-blue-500' : 'text-slate-900 dark:text-white'}`}>
                                        {schedule.startTime} - {schedule.endTime}
                                    </div>
                                    {isOngoing && (
                                        <div className="text-[11px] font-medium text-blue-500 mt-0.5 animate-pulse">
                                            🔴 LIVE NOW
                                        </div>
                                    )}
