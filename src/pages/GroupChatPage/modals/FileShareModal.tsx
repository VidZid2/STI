 ...
 * Design inspired by file-upload.tsx component
 */
...
// Animation variants matching file-upload.tsx
const mainVariant = {
    initial: { x: 0, y: 0 },
...
// SVG Icons
const UploadIcon = ({ size = 24, color = '#6366F1' }: { size?: number; color?: string }) => (
...
const FileIcon = ({ size = 16, color = '#6366F1' }: { size?: number; color?: string }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
...
const ImageIcon = ({ size = 16, color = '#10B981' }: { size?: number; color?: string }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
...
export const FileShareModal: React.FC<FileShareModalProps> = ({ isOpen, onClose, onSend, colors }) => {
    // VERSION 4 - Debug logs
    console.log('$$$ FileShareModal RENDER VERSION 4 $$$');
    ...
    const handleFileSelect = useCallback((newFiles: File[]) => {
        console.log('=== FileShareModal handleFileSelect called ===');
        ...
        const fileDataPromises = filesToAdd.map(file => {
            return new Promise<FileData>((resolve) => {
                // Determine file type - use file.type or infer from extension
                ...
                if (isImageFile(fileType, file.name)) {
                    ...
                    reader.onload = (e) => {
                        ...
                        console.log('Preview generated for:', file.name, 'preview length:', fileData.preview?.length);
                        resolve(fileData);
                    };
                    reader.onerror = () => {
                        console.log('Error generating preview for:', file.name);
                        resolve(fileData);
                    };
        ...
        Promise.all(fileDataPromises).then(newFileData => {
            console.log('All file data processed:', newFileData);
            setFiles(prev => {
                const updated = [...prev, ...newFileData].slice(0, MAX_FILES);
    ...
    const handleShare = () => {
        console.log('$$$ FileShareModal handleShare VERSION 4 $$$');
        console.log('files:', files);
        console.log('files.length:', files.length);
        
        if (files.length > 0) {
            const file = files[0];
            console.log('First file:', file.name, 'type:', file.type);
            console.log('First file preview exists:', !!file.preview);
            
            // Pass all file data including preview for images
            ...
            console.log('$$$ Calling onSend directly (not ref) $$$');
            console.log('onSend:', onSend);
            
            // Call onSend directly
    ...
    // Cleanup on unmount - no need to revoke base64 data URLs
    useEffect(() => {
        return () => {
            setFiles([]);
        };
            ...
            >
                ...
                <motion.div
                    ...
                    style={{
                        ...
                        background: files.length === 0 ? 'rgba(249, 250, 251, 0.5)' : 'rgba(249, 250, 251, 0.5)',
                    ...
                    <input
                        ...
                        onChange={(e) => {
                            ...
                            if (selectedFiles.length > 0) {
                                console.log('Calling handleFileSelect with', selectedFiles.length, 'files');
                            ...
                            >
                                <AnimatePresence mode="popLayout">
                                    ...
                                    {files.slice(0, MAX_VISIBLE_FILES).map((file, idx) => (
                                        ...
                                        >
                                            {/* Image Preview */}
                                            {file.preview && isImageFile(file.type, file.name) && (
                                                <div style={{
                                                    width: '100%',
                                                    height: '120px',
                                                    borderRadius: '8px',
                                            ...
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                                ...
                                                <div style={{ textAlign: 'left' }}>
                                                    <p style={{ fontSize: '14px', fontWeight: 500, color: '#1d4ed8', margin: 0 }}>
                                                        +{additionalFilesCount} {additionalFilesCount === 1 ? 'file' : 'files'}
                                                    </p>
                ...
                {/* Share Button - Centered, Yellow theme matching New Group button style */}
