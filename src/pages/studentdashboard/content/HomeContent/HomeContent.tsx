            ...
            >
                                ...
                                <div className="flex items-center gap-3 sm:gap-5 w-full sm:w-auto">
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <div className={`absolute -bottom-1 left-1/2 -translate-x-1/2 min-w-[36px] h-[18px] px-1.5 rounded-md flex items-center justify-center text-[10px] font-bold tracking-wider shadow-sm border-[2px] z-20 transition-colors duration-300 ${level >= 20 ? 'bg-yellow-400 text-blue-800' : 'bg-blue-500 text-white'} ${isDarkMode ? (showOnlineStatus ? 'border-emerald-400' : 'border-slate-800') : (showOnlineStatus ? 'border-emerald-500' : 'border-white')}`}>
                                                <span className="ml-[0.05em]">{level >= 20 ? 'MAX' : `LV.${level}`}</span>
                                            </div>

                                            {/* Premium Crown Badge on top of Avatar */}
                                            {level >= 20 && (
                                                <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 z-30 pointer-events-none drop-shadow-[0_3px_5px_rgba(0,0,0,0.4)]">
                                                    <svg width="42" height="42" viewBox="0 0 42 42" fill="none" className="w-7 h-7 sm:w-8 sm:h-8 rotate-[-5deg]">
                                                        ...
                                                        <path d="M32.2491 24.9696C32.5251 24.0481 33.1715 23.2827 34.0337 22.8563L35.4324 22.1645C36.0413 21.8634 36.7138 22.4402 36.5088 23.0878L33.5556 32.4189C33.4542 32.7393 33.1569 32.957H32.8208 32.957H30.8916C30.3755 32.957 30.0052 32.4596 30.1533 31.9652L32.2491 24.9696Z" fill="#F59E0B"/>
                                    ...
                                    <div className="flex flex-col sm:hidden justify-center min-w-0 flex-1">
                                        ...
                                        <div className="flex items-center gap-2 flex-wrap">
                                            ...
                                            {level >= 8 && !weatherLoading && weather ? (
                ...
                <div className="flex flex-col gap-6 relative z-10 w-full">
                    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm rounded-[24px] flex flex-col lg:flex-row w-full overflow-hidden transition-all duration-300 hover:shadow-md">
                        ...
                        <div className="flex-1 p-5 lg:p-8 flex flex-col gap-6 group hover:bg-slate-50/30 dark:hover:bg-slate-800/50 transition-colors">
                            ...
                            >
                                <img 
                                    src="/images/PLaceholder for UPDATE CHANGELOG.png" 
                                    alt="eLMS 2.0 Update Changelog" 
                ...
                <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 relative z-10 w-full max-w-6xl mx-auto items-start">
                    ...
                    <div className="flex flex-col gap-6 w-full lg:w-[380px] xl:w-[420px] flex-shrink-0 sticky top-6">
                        ...
                        <div className="order-1 lg:order-3 hidden sm:grid lg:hidden grid-cols-2 gap-3 w-full">
                            {/* Tools */}
                        ...
                        {/* Quick View Widgets Slot (Desktop Only) - Fixed height for 'Pantay' alignment */}
                        <div className="order-4 hidden lg:flex w-full rounded-[24px] mt-2 flex-col h-[350px] lg:h-[559px] relative group/qv">
                            {/* Inner container with overflow-hidden for GradualBlur clipping */}
                            <div className="w-full h-full bg-white dark:bg-slate-800 rounded-[24px] p-5 sm:p-6 flex flex-col gap-1 relative overflow-hidden">
                                
                                {/* Widgets Area (Scrolling handled internally by WidgetSidebar) */}
                                <div className="flex-1 overflow-hidden min-h-0 -mx-1 px-1 flex flex-col">
                                    {quickViewSlot}
                                </div>

                                {/* Gradual Blur overlaying the bottom of the card seamlessly */}
                                {quickViewSlot && (
                                    <GradualBlur
                                        position="bottom"
                                        height="4.5rem"
                                        ...
                                        curve="ease-out"
                                        zIndex={10}
                                        style={{ bottom: 0, left: 0, right: 0, width: '100%', borderRadius: '0 0 24px 24px' }}
                                    />
                                )}
                            </div>
                            {/* Border + Shadow Overlay — sits OUTSIDE overflow-hidden so shadow renders on all corners */}
                            <div className="absolute inset-0 rounded-[24px] border border-slate-200 dark:border-slate-700 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] group-hover/qv:shadow-md transition-shadow duration-300 pointer-events-none z-20"></div>
                        </div>
                    </div>
                                    ...
                                    <div className="flex flex-col gap-4">
                                        ...
                                        <div className="bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 shadow-sm rounded-[24px] p-4 flex flex-col gap-4 group/grade transition-all duration-300 hover:shadow-md hover:border-blue-300 dark:hover:border-blue-700 cursor-default">
                                            ...
                                            <div className="flex flex-col gap-2.5">
                                                <div className="flex flex-nowrap justify-between items-center gap-1.5 sm:gap-3 w-full">
                                                    <div className="flex-1 min-w-0 flex items-center justify-center sm:justify-start gap-1.5 sm:gap-2 px-2 sm:px-3 py-1.5 bg-white dark:bg-slate-800 rounded-[10px] sm:rounded-[12px] border border-slate-200 dark:border-slate-700/50 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] transition-all duration-300 hover:shadow-md hover:-translate-y-0.5 cursor-default group/tag">
                                                        <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-[6px] sm:rounded-[8px] bg-blue-50 dark:bg-blue-900/50 flex items-center justify-center text-blue-600 dark:text-blue-400 group-hover/tag:scale-110 transition-transform duration-300 shrink-0">
                                                            <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                                                        </div>
                                                        <span className="text-[10px] sm:text-xs font-bold text-slate-800 dark:text-slate-200 uppercase tracking-tight sm:tracking-wider whitespace-nowrap overflow-hidden text-ellipsis">{currentProgressPercentage}% Complete</span>
                                                    </div>
                                                    <div className="flex-1 min-w-0 flex items-center justify-center sm:justify-start gap-1.5 sm:gap-2 px-2 sm:px-3 py-1.5 bg-white dark:bg-slate-800 rounded-[10px] sm:rounded-[12px] border border-slate-200 dark:border-slate-700/50 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] transition-all duration-300 hover:shadow-md hover:-translate-y-0.5 cursor-default group/tag">
                                                        <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-[6px] sm:rounded-[8px] bg-slate-100 dark:bg-slate-800/80 flex items-center justify-center text-slate-500 dark:text-slate-400 group-hover/tag:scale-110 transition-transform duration-300 shrink-0">
                                                            <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><line x1="9" y1="3" x2="9" y2="21"/></svg>
                                                        </div>
                                                        <span className="text-[10px] sm:text-xs font-bold text-slate-800 dark:text-slate-200 tracking-tight sm:tracking-wider whitespace-nowrap overflow-hidden text-ellipsis">{currentModulesRemaining} modules left</span>
                                                    </div>
                                                </div>
                                                <div className="w-full h-3 rounded-full bg-slate-200/80 dark:bg-slate-700 overflow-hidden shadow-[inset_0_1.5px_3px_rgba(0,0,0,0.15)] border border-black/5 dark:border-white/5">
