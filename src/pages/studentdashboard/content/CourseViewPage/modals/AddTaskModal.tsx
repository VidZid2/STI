...
const TASK_CATEGORIES: { id: TaskCategory; label: string; icon: React.ReactNode; color: string }[] = [
    { id: 'assignment', label: 'Assignments', icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>, color: 'blue' },
    { id: 'performance', label: 'Performances', icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>, color: 'yellow' },
    { id: 'quiz', label: 'Quizzes', icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>, color: 'purple' },
    { id: 'practical', label: 'Practicals', icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>, color: 'green' },
    { id: 'journal', label: 'Journals', icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>, color: 'orange' },
];
...
interface AddTaskModalProps {
    ...
    setNewTaskPoints: (v: string) => void;
    newTaskInstructions: string;
    setNewTaskInstructions: (v: string) => void;
    newTaskFiles: File[];
    setNewTaskFiles: (v: File[]) => void;
    isCreatingTask: boolean;
    setIsCreatingTask: (v: boolean) => void;
}

const AddTaskModal: React.FC<AddTaskModalProps> = ({
    isOpen, onClose, courseId, isTeacherMode,
    ...
    newTaskInstructions, setNewTaskInstructions,
    newTaskFiles, setNewTaskFiles,
                    ...
                    >
                        ...
                        <div className="flex-1 overflow-y-auto p-5 space-y-4">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className={`w-full h-10 px-3 text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 ${isTeacherMode ? 'bg-zinc-800 border-zinc-700 text-white placeholder-zinc-500' : 'bg-white border border-zinc-200 text-zinc-900 placeholder-zinc-400'}`}
                                />
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className={`w-full h-10 px-3 text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 ${isTeacherMode ? 'bg-zinc-800 border-zinc-700 text-white [color-scheme:dark]' : 'bg-white border border-zinc-200 text-zinc-900'}`}
                                    />
                                </div>
                                <div>
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    rows={2}
                                    className={`w-full px-3 py-2 text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 resize-none ${isTeacherMode ? 'bg-zinc-800 border-zinc-700 text-white placeholder-zinc-500' : 'bg-white border border-zinc-200 text-zinc-900 placeholder-zinc-400'}`}
                                />
                            </div>
                                ...
                                >
                                    <AnimatePresence mode="wait">
                                        {newTaskFiles.length === 0 ? (
                                            <motion.div key="upload-area" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center">
                                                <div className="relative w-full max-w-xl mx-auto mb-4 flex items-center justify-center">
                                                    <div className="relative h-20 w-20">
                                                        <motion.div
                                                            variants={{ initial: { x: 0, y: 0 }, animate: { x: 20, y: -20, opacity: 0.9 } }}
                                                        ...
                                                        <motion.div variants={{ initial: { opacity: 0 }, animate: { opacity: 1 } }} className="absolute top-0 left-0 opacity-0 border-2 border-dashed border-blue-400 z-30 bg-transparent h-20 w-20 rounded-xl" />
                                                    </div>
                                                </div>
                                            ...
                                            <motion.div key="file-list" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex flex-col gap-2">
                                                <AnimatePresence mode="popLayout">
                                                    {newTaskFiles.slice(0, 2).map((file, idx) => (
                                                        <motion.div
                                                            key={"file" + idx}
                                                            layout
                                                            initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10, scale: 0.95 }}
                                                        ...
                                                        >
                                                            <div className="flex justify-between w-full items-center gap-3">
                                                                ...
                                                                <div className="flex items-center gap-2">
                                                                    ...
                                                                    <button
                                                                        onClick={(e) => { e.stopPropagation(); setNewTaskFiles(newTaskFiles.filter((_, i) => i !== idx)); }}
                                                    ...
                                                    {newTaskFiles.length > 2 && (
                                                        <motion.div key="collapsed-summary" layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10, scale: 0.95 }} transition={{ type: "spring", stiffness: 300, damping: 25 }}
                                                        ...
                                                        >
                                                            <div className="flex items-center gap-3">
                                                                ...
                                                                <div className="text-left">
                                                                    <p className={`text-xs font-medium ${isTeacherMode ? 'text-blue-400' : 'text-blue-700'}`}>+{newTaskFiles.length - 2} more {newTaskFiles.length - 2 === 1 ? 'file' : 'files'}</p>
                                                                    <p className={`text-[10px] ${isTeacherMode ? 'text-blue-500' : 'text-blue-500'}`}>{(newTaskFiles.slice(2).reduce((acc, f) => acc + f.size, 0) / (1024 * 1024)).toFixed(2)} MB total</p>
                                                                </div>
                                                            </div>
                                                        </motion.div>
                                                    )}
                        ...
                        <div className={`px-5 py-3 border-t flex items-center justify-end gap-2 ${isTeacherMode ? 'border-zinc-700' : 'border-zinc-100'}`}>
                            ...
                            <motion.button
                                ...
                                className={`px-4 py-2 text-sm font-semibold text-white rounded-lg transition-all flex items-center gap-1.5 ${!newTaskTitle || !newTaskDueDate || isCreatingTask ? 'bg-blue-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`}
                            >
                                {isCreatingTask ? (
                                    <>
