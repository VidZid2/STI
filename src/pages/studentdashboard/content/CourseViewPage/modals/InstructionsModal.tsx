...
interface InstructionsModalTask {
    title: string;
    description?: string;
    instructions?: string;
...
interface InstructionsModalProps {
    task: InstructionsModalTask | null;
    onClose: () => void;
}
                        ...
                        <div className="px-8 pb-8 pt-6 overflow-y-auto custom-scrollbar flex-1 bg-white">
                            <div className="space-y-6">
                                ...
                                {(task.allowLateSubmission !== undefined ||
                                    (task.maxAttempts !== undefined && task.maxAttempts > 1) ||
                                    task.rubricEnabled) && (
                                    <div className="border border-zinc-200/80 rounded-2xl p-6 bg-zinc-50/30">
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                            ...
                                            <div style={{
                                                ...
                                                background: task.allowLateSubmission ? 'rgba(245, 158, 11, 0.08)' : 'rgba(239, 68, 68, 0.06)',
                                                border: task.allowLateSubmission ? '1px solid rgba(245, 158, 11, 0.15)' : '1px solid rgba(239, 68, 68, 0.12)',
                                            }}>
                                                ...
                                                <div>
                                                    <div style={{ fontSize: '13px', fontWeight: 600, color: task.allowLateSubmission ? '#92400e' : '#991b1b' }}>
                                                        {task.allowLateSubmission ? 'Late submissions allowed' : 'No late submissions'}
                                                    </div>
                                                    {task.allowLateSubmission && task.latePenalty && task.latePenalty > 0 && (
                                            ...
                                            {task.rubricEnabled && (
                                                <div style={{ padding: '14px', borderRadius: '12px', background: 'rgba(16, 185, 129, 0.06)', border: '1px solid rgba(16, 185, 129, 0.12)' }}>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: task.rubricCriteria && task.rubricCriteria.length > 0 ? '12px' : '0' }}>
                                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#10b981" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                                            <rect x="3" y="3" width="18" height="18" rx="2" ry="2" />
                                                            <line x1="3" y1="9" x2="21" y2="9" />
                                                            <line x1="9" y1="21" x2="9" y2="9" />
                                                        </svg>
                                                        <div style={{ fontSize: '13px', fontWeight: 600, color: '#065f46' }}>Graded with rubric</div>
                                                    </div>
                                                    {task.rubricCriteria && task.rubricCriteria.length > 0 && (
                                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                                            {task.rubricCriteria.map((criterion, idx) => (
                                                                <div key={criterion.id || idx} style={{ padding: '10px 12px', borderRadius: '8px', background: 'rgba(255, 255, 255, 0.7)', border: '1px solid rgba(16, 185, 129, 0.1)' }}>
                                                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: criterion.description ? '4px' : '0' }}>
                                                                        <span style={{ fontSize: '12px', fontWeight: 600, color: '#334155' }}>{criterion.name}</span>
                                                                        <span style={{ fontSize: '11px', fontWeight: 600, padding: '2px 8px', borderRadius: '6px', background: 'rgba(16, 185, 129, 0.1)', color: '#10b981' }}>
                                                                            {criterion.points} pts
                                                                        </span>
                                                                    </div>
                                                                    {criterion.description && (
                                                                        <p style={{ fontSize: '11px', color: '#64748b', margin: 0, lineHeight: 1.4 }}>{criterion.description}</p>
                                                                    )}
                                                                </div>
                                                            ))}
                                                        </div>
