...
export const FilePreviewCard: React.FC<FilePreviewCardProps> = ({ attachment, isOwn }) => {
    ...
    // Check if URL is valid (not empty or a placeholder)
    const hasValidImageUrl = imageUrl && imageUrl.length > 0 && !imageUrl.startsWith('[');
    
    if (isImage && hasValidImageUrl) {
        const handleImageClick = (e: React.MouseEvent) => {
            e.stopPropagation();
            e.preventDefault();
            if (!imageError) setShowFullImage(true);
        };
                ...
                >
                    ...
                    {imageError && (
                        <div style={{
                            ...
                            <span style={{ fontSize: '10px' }}>Failed</span>
                        </div>
                    )}
                    <img
                        src={imageUrl}
    ...
    // Image without valid URL (loaded from database) - show placeholder
    if (isImage && !hasValidImageUrl) {
        return (
            <div
                style={{
